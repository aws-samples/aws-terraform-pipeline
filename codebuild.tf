...
module "plan" {
  ...
  build_timeout         = var.build_timeout
  build_spec            = try(var.build_override["plan_buildspec"], file("${path.module}/modules/codebuild/buildspecs/plan.yml"))
  log_group             = aws_cloudwatch_log_group.this.name
  image                 = can(var.build_override["plan_image"], "hashicorp/terraform:${var.terraform_version}")
  vpc                   = var.vpc
}
...
module "apply" {
  ...
  environment_variables = local.env_var
  build_timeout         = var.build_timeout
  build_spec            = try(var.build_override["apply_buildspec"], file("${path.module}/modules/codebuild/buildspecs/apply.yml"))
  log_group             = aws_cloudwatch_log_group.this.name
  image                 = can(var.build_override["apply_image"], "hashicorp/terraform:${var.terraform_version}")
  vpc                   = var.vpc
}
